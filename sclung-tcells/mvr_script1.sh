...
for dir in $(aws s3 ls s3://amaynard-bucket/170129_NS500126_0650_AHVM75BGXY/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line4" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170129_NS500126_0650_AHVM75BGXY/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/170129_NS500126_0650_AHVM75BGXY/$fileName2 --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://amaynard-bucket/170202_NS500126_0653_AHVL5NBGXY/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line4" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170202_NS500126_0653_AHVL5NBGXY/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/170202_NS500126_0653_AHVL5NBGXY/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://amaynard-bucket/170215_NS500126_0658_AH7TLYBGX2/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line4" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170215_NS500126_0658_AH7TLYBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/170215_NS500126_0658_AH7TLYBGX2/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line4" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/170504_NS500126_0691_AHC22JBGX2/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://amaynard-bucket/170508_NS500126_0693_AHC2MFBGX2/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/70508_NS500126_0693_AHC2MFBGX2/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://amaynard-bucket/170510_NB501938_0003_AHM7KMBGX2/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/170510_NB501938_0003_AHM7KMBGX2/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://lincoln.harris-work/171120_A00111_0085_AH57YYDMXX/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList2.txt); do
		...
		if [ "$dirName1" == "$line4" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://lincoln.harris-work/171120_A00111_0085_AH57YYDMXX/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/171120_A00111_0085_AH57YYDMXX/$dir --recursive
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://lincoln.harris-work/171120_A00111_0086_BH55NVDMXX/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList2.txt); do
		...
		if [ "$dirName1" == "$line4" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://lincoln.harris-work/171120_A00111_0086_BH55NVDMXX/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/171120_A00111_0086_BH55NVDMXX/$dir --recursive
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://czbiohub-seqbot/fastqs/180226_A00111_0105_AH3KCHDSXX/rawdata | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/180226_A00111_0105_AH3KCHDSXX/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://czbiohub-seqbot/fastqs/180307_A00111_0109_BH3KCJDSXX/rawdata/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/180307_A00111_0109_BH3KCJDSXX/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://czbiohub-seqbot/fastqs/180319_A00111_0119_AH3JYJDSXX/rawdata/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/180319_A00111_0119_AH3JYJDSXX/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://czbiohub-seqbot/fastqs/180320_NB501961_0086_AHLGLGBGX5/rawdata/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/180320_NB501961_0086_AHLGLGBGX5/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://czbiohub-seqbot/fastqs/180405_A00111_0129_BH3VFYDSXX/rawdata/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/180405_A00111_0129_BH3VFYDSXX/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
...
for dir in $(aws s3 ls s3://czbiohub-seqbot/fastqs/180423_A00111_0141_AH3VFNDSXX/rawdata/ | awk '{print $2}'); do
	...
	for line in $(cat tCellList.csv); do
		...
		if [ "$fileName" == "$line" ]; then
			...
			echo 'match found'
			echo $line4
			aws s3 cp s3://amaynard-bucket/170504_NS500126_0691_AHC22JBGX2/$dir s3://darmanis-group/singlecell_lungadeno/tcell/fastqs/180423_A00111_0141_AH3VFNDSXX/$dir --recursive --exclude '*' --include '*.gz'
			echo " "
		fi
	done
done
